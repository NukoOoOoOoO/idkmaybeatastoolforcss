...
T WordSwapAsm( T w )
{
    __asm { mov ax, w xchg al, ah }
}

template <typename T>
T DWordSwapAsm( T dw )
{
    __asm { mov eax, dw bswap eax }
}
...
using BitBufErrorType = enum
{
    BITBUFERROR_VALUE_OUT_OF_RANGE = 0,
    // Tried to write a value with too few bits.
    BITBUFERROR_BUFFER_OVERRUN,
    // Was about to overrun a buffer.
...
public:
    ...
    {
        return static_cast<char>(ReadUBitLong( 8 ));
    }
    ...
    {
        return static_cast<short>(ReadUBitLong( 16 ));
    }
